...
<body>
    <div class="page-wrapper">
        ...
        <section class="info-section">
            <div class="info-card">
                <h3>🏆 Leaderboard</h3>
                <div id="leaderboard" class="leaderboard"><p style="color:rgba(224,230,240,0.5);text-align:center;padding:20px;">Loading...</p></div>
                <div class="lb-submit">
            ...
            <div class="info-card"><h3>🚀 Coming Soon</h3><p>Automatic score detection, achievements, and weekly tournaments. Stay tuned!</p></div>
        </section>
    ...
    <script>
        document.addEventListener("DOMContentLoaded",()=>{
            const container=document.getElementById("game-container");const loading=document.getElementById("loading");
            const ruffle=window.RufflePlayer.newest();const player=ruffle.createPlayer();
            player.style.width="100%";player.style.height="100%";container.appendChild(player);
            player.ruffle().load({url:"/assets/brain_game_2_6_7_translated_v1.swf",autoplay:"on",unmuteOverlay:"visible",backgroundColor:"#000000",letterbox:"on",warnOnUnsupportedContent:false,contextMenu:"rightClickOnly"}).then(()=>{loading.classList.add("hidden");}).catch(err=>{loading.innerHTML='<div class="brain-icon">⚠️</div><div class="loading-text" style="color:var(--neon-pink);">Failed to load</div>';});
            loadLeaderboard();
        });
        ...
        async function loadLeaderboard(){
            ...
            if(d.success&&d.leaderboard.length){
                document.getElementById('leaderboard').innerHTML=d.leaderboard.slice(0,10).map((e,i)=>'<div class="lb-entry"><span class="lb-rank">'+(i<3?['🥇','🥈','🥉'][i]:'#'+(i+1))+'</span><span class="lb-name">'+e.name+'</span><span class="lb-score">'+e.score+'</span></div>').join('');
            }else{document.getElementById('leaderboard').innerHTML='<p style="color:rgba(224,230,240,0.5);text-align:center;padding:20px;">No scores yet. Be the first!</p>';}}catch(e){document.getElementById('leaderboard').innerHTML='<p style="color:rgba(224,230,240,0.5);text-align:center;padding:20px;">Could not load leaderboard</p>';}
        }
        async function submitScore(){
            const name=document.getElementById('playerName').value.trim();
            const score=parseInt(document.getElementById('playerScore').value);
            if(!name||!score||score<0){alert('Please enter your name and a valid score');return;}
            try{const r=await fetch('/api/score',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,score})});
            const d=await r.json();if(d.success){alert('Score submitted! Rank: #'+d.rank);loadLeaderboard();document.getElementById('playerScore').value='';}}catch(e){alert('Failed to submit score');}
        }
    <\/script>
</body>
</html>`;
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/leaderboard') {
      try {
        ...
        return new Response(JSON.stringify({ success: true, leaderboard: data || [] }), { headers: { 'Content-Type': 'application/json' } });
      } catch { return new Response(JSON.stringify({ success: true, leaderboard: [] }), { headers: { 'Content-Type': 'application/json' } }); }
    }
    ...
    if (url.pathname === '/api/score' && request.method === 'POST') {
      try {
        ...
        const entry = { id: crypto.randomUUID(), name: String(name).slice(0, 20), score: Math.floor(score), timestamp: Date.now() };
        let lb = []; try { lb = await env.LEADERBOARD.get('leaderboard_v1', { type: 'json' }) || []; } catch {}
        lb.push(entry); lb.sort((a, b) => b.score - a.score); lb = lb.slice(0, 100);
        await env.LEADERBOARD.put('leaderboard_v1', JSON.stringify(lb));
        return new Response(JSON.stringify({ success: true, entry, rank: lb.findIndex(e => e.id === entry.id) + 1 }), { headers: { 'Content-Type': 'application/json' } });
    ...
    // Serve SWF from GitHub
    if (url.pathname === "/assets/brain_game_2_6_7_translated_v1.swf") {
      const res = await fetch(SWF_URL);
      return new Response(res.body, { headers: { "Content-Type": "application/x-shockwave-flash", "Access-Control-Allow-Origin": "*", "Cache-Control": "public, max-age=86400" } });
    }
    
    // Serve PNG from GitHub
